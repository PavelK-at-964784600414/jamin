...
export default function SideNav() {
  return (
    <div className="flex h-full flex-col px-3 py-4 md:px-2">
      ...
      <div className="flex grow flex-row justify-between space-x-2 md:flex-col md:space-x-0 md:space-y-2 bg-black-600">
        <NavLinks />
        <div className="hidden h-auto w-full grow rounded-md bg-gray-50 md:block bg-black"></div>
        <form action={async () => {
            'use server';
            await signOut();
          }}
        ...
        <button className="flex h-[48px] w-full grow items-center justify-center gap-2 rounded-md bg-gray-50 p-3 text-sm font-medium hover:bg-orange-400 bg-orange-600 hover:text-blue-400 text-black md:flex-none md:justify-start md:p-2 md:px-3">
            <PowerIcon className="w-6 text-black hover:text-blue-600" />
            <div className="hidden md:block hover:text-blue-600 text-black">Sign Out</div>
          </button>
        </form>
      </div>
    </div>
